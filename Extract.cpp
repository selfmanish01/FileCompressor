// This program reads a zip file which ends with '#' & extracts the file.
#include <stdio.h>
#include <iostream>
#include <fstream>
...
int main()
{
    input.open("Zip.txt"); // opening the file

    if (!input.is_open())
    { // when the file name is wrong
    ...
    int S,     // the starting char in the ascii table
        int E; // the ending char in the ascii table
    ...
    for (int i = 0; i < E - S; i++)
    { // taking the input of number of characters
        input >> num[i];
        // cout << num [i] << " "; ///test
    }
    // cout << endl; ///test
...
void treeBuilder(int *num, node *tree, int x, int S, int E)
{
    ...
    for (; position < E - S; position++)
    { // setting the value of the initial nodes
    ...
    {                               // building up the tree
        ...
        for (int i = 0; i < x; i++)
        { // finds left sub tree (smallest value)
            if (tree[i].r == NULL && tree[i].value >= 0)
                if (tree[i].value < tree[lt].value)
                    lt = i;
        }
        ...
        for (int i = 0; i < x; i++)
        { // finds right sub tree (second smallest value)
            if (tree[i].r == NULL && tree[i].value >= 0)
                if (tree[i].value < tree[rt].value)
                    rt = i;
        }
...
{
    FILE *output;
    output = fopen("ExtractedText.txt", "w");

    char ch;
    input >> ch;
    while (ch != '#')
    {                       // reading the file & counting the characters
